...
this.insert(parent, null, true);
	...
	_hasFormScrollContainer: function(cell) {
		var element = cell;
		while (element && element !== document.body) {
			if (element.id && element.id.indexOf('.form_scroll') !== -1) {
				return true;
			}
	...
	_hasReportContainer: function(cell) {
		var element = cell;
		while (element && element !== document.body) {
			if (element.className && (
				element.className.indexOf('report_container') !== -1 ||
				element.className.indexOf('sys_report') !== -1 ||
				element.className.indexOf('report_content') !== -1
			)) {
				return true;
			}
			if (element.id && element.id.indexOf('report') !== -1) {
				return true;
			}
			element = element.parentElement;
		}
	...
	_isHierarchicalFallback: function(cell) {
		try {
			var formList = $j(cell).closest('div.list_div');
			return formList.length > 0 && formList.hasClass('hierarchical');
		} catch (e) {
			return false;
		}
	},
	...
	_getHierarchicalDirectLeft: function(cell) {
		var cellRect = cell.getBoundingClientRect();
		var scrollLeft = window.pageXOffset || document.documentElement.scrollLeft || document.body.scrollLeft || 0;
		return cellRect.left + scrollLeft;
	},
	
	_getOffsetLeft: function(cell) {
		var isHierarchical = this.editor.tableController.isHierarchical() || this._isHierarchicalFallback(cell);
		var hasReportContainer = this._hasReportContainer(cell);
		if (isHierarchical || hasReportContainer) {
			return this._getHierarchicalDirectLeft(cell);
		}
	...
	_getHierarchicalDirectTop: function(cell) {
		var cellRect = cell.getBoundingClientRect();
		var scrollTop = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop || 0;
		return cellRect.top + scrollTop;
	},
	
	_getOffsetTop: function(cell) {
		var isHierarchical = this.editor.tableController.isHierarchical() || this._isHierarchicalFallback(cell);
		var hasFormScrollContainer = this._hasFormScrollContainer(cell);
		var hasReportContainer = this._hasReportContainer(cell);
		if (isHierarchical && !hasFormScrollContainer || hasReportContainer) {
			return this._getHierarchicalDirectTop(cell);
		}
