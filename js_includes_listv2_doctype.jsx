...
if (this.isHierarchical())
	...
	_handleADPEncryptedCells: async function() {
		try {
			if (typeof GlideZTE === 'undefined') {
				return;
			}
			...
			var tdElements = document.querySelectorAll('td[adp_encrypted="true"]');
			for (let i = 0; i < tdElements.length; i++) {
				var encryptedValue = tdElements[i].getAttribute('title');
				if (encryptedValue === null) {
					encryptedValue = tdElements[i].getAttribute('data-original-title');
				}
				if (encryptedValue === null) {
					encryptedValue = tdElements[i].textContent;
				}
				if(this._isEncrypted(encryptedValue)) {
					var decryptedValue = await this._decryptValue(encryptedValue);
					if (encryptedValue === decryptedValue) {
						continue;
					}
					var aElement = tdElements[i].querySelector('a');
					if (aElement) {
						aElement.textContent = decryptedValue;
					} else {
						tdElements[i].textContent = decryptedValue;
					}
					tdElements[i].setAttribute('title', decryptedValue);
					tdElements[i].setAttribute('data-original-title', decryptedValue);
				}
			}
		} catch (error) {
			jslog("Error occurred in GlideZTE: " + error);
			return;
		}
	},
	_decryptValue: async function(encryptedValue) {
		try {
			var decryptedValue = await GlideZTE?.()?.getSymmetricCrypto?.()?.decryptByKeyId?.(encryptedValue);
			if (decryptedValue) {
				return decryptedValue;
			}
			return encryptedValue;
		} catch (error) {
			jslog("Error decrypting ADP encrypted value: " + error);
			return encryptedValue;
		}
	},
	_isEncrypted: function(value) {
...
return false;
	...
	_getOnHideScript: function() {
		return function() {
			var firingObj = this.getFiringObject();
			if (firingObj)
				firingObj.setAttribute('aria-expanded', false);
		}
	},
	onRowContextMenu: function(element, ev) {
		...
		if (element.hasAttribute('glide_value'))
			variables['g_fieldValue'] = element.getAttribute('glide_value');
...
_getRefreshParms: function(filter,  setAsDefaultFlag) {
		...
		if (related) {
			...
			var key = encodeURIComponent(this.getParentTable() + "." + parentSysId + "." + related);
...
this.listID = this.listID + '_' + colmn;
	...
	_toggleGroupButtonLabel: function(button, collapsed) {
		var labelMessage = collapsed ? getMessage("Expand group") : getMessage("Collapse group");
		var groupFieldValue = button?.dataset?.groupFieldValue;
		if (groupFieldValue) {
			labelMessage += " " + groupFieldValue;
		}
...
img.src = "images/list_v2_heir_hide.gifx";
        ...
        var directionIcon = g_text_direction == "rtl" ? "icon-vcr-left" : "icon-vcr-right";
	    if (show) {		
			img.removeClassName(directionIcon);
			img.addClassName("icon-vcr-down");
        } else {
			img.addClassName(directionIcon);
...
_refreshAjax: function( firstRow,  additionalParms, includeOrderBy) {
		...
		var firstRowKey = encodeURIComponent(this._createFirstRowKey());
...
var loadingMessage = getMessage("Loading...");
GlideList2.LOADING_MSG =
  "<div class='list_loading_div'>" +
  loadingMessage +
"<img src='images/ajax-loader.gifx' alt='" +
  loadingMessage +
...
_checkAction: function(opt,  sysIds) {
	...
	runAction: function(select) {
		...
		if (getAttributeValue(opt, 'client') == 'true') {
			...
			if (window.__metadata_trace_enabled)
				window.delegatedActionPushForMetadataTracing('click', 'sys_ui_action', id, null);
...
if (evt.keyCode !== 32 )
	...
	document.body.on('click', 'button[data-ux-list-metrics-ui-action]', function (evt, element) {
		var options = {
			actionId: element.getAttribute('gsft_id'),
			table: element.getAttribute('table'),
			label: element.dataset.actionLabel
		}
		GlideUXMetrics && GlideUXMetrics.track('Click List Action', options);
	})
...
var orderByMatches = query.match(/\^ORDERBY/g) !== null ? query.match(/\^ORDERBY/g).length : 0;
			...
			for (var i = 0; i < parts.length; i++) {
				...
				var fieldType = $table.find('tr.list_header_search_row td[name="'+p.field+'"]').attr("data-glide-type");
				var isBoolean = fieldType === "boolean";
				if (isBoolean) {
					if(value === "1")
				...
				if (p.Goto == 'true')
					hasGotoQuery = true;
				if (setInputValue($table, p.field, buildDisplayValue(p.operator, unescapeQueryValue(p.operator,value), fieldType)))
	...
	function getQueryFromTable($table, tableName) {
		...
		$table.find('tr.list_header_search_row td').each(function(index, el) {
			...
			if ($input.length === 0)
				return;
			var value = $input.val() || '';
			var fieldType = el.getAttribute('data-glide-type');
			var isBoolean = fieldType === "boolean";
			var isNumeric = fieldType === "integer" || fieldType === "decimal";
			var newTerm;
			var choiceQuery = $input.data('choice_query');
			if (choiceQuery)
				newTerm = choiceQuery;
			else
				newTerm = buildTerm(field, value, isBoolean, isNumeric);
			var found = false;
	...
	function buildTerm(field, value, isBoolean, isNumeric) {
		var operator = (isBoolean || isNumeric) ? '=' : 'STARTSWITH';
		var trueValue;
		if (value.indexOf('*') == 0 || value.indexOf('.') == 0) {
			...
			operator = 'NOT LIKE';
			value = value.substring(2);
		} else if (value.indexOf('>=') == 0 && isNumeric) {
			operator = '>=';
			value = value.substring(2);
		} else if (value.indexOf('>') == 0 && isNumeric) {
			operator = '>';
...
  		}
if (isNumeric)
  			operator = ["=", "!=", "<", "<=", ">", ">="].indexOf(operator) > -1 ? operator : "=";
		return {
			field: field,
			operator: operator,
			value: value
	...
	function buildDisplayValue(operator, value, fieldType) {
		switch(operator) {
			case 'LIKE':
				return '*' + value;
			case 'STARTSWITH':
...
var GlideWidgetFilter = Class.create(GlideListWidget, {
	...
	toggleFilter: function() {
		...
		if (filterDiv.getAttribute('gsft_empty') === 'true') {
			this._loadFilter(filterDiv, listDiv);
			return;
		}
		var shouldShowFilter = filterDiv.style.display == "none";
		this._filterDisplay(shouldShowFilter);
		this._setFocusToolbar(shouldShowFilter);
	},
	togglePin: function() {
...
if (!bc_hidden.innerHTML)
	...
	_filterDisplay: function(shouldShowFilter) {
		...
		this._changeFilterToggleIcon(e, shouldShowFilter);
		
		writeTitle(e, this._getFilterIconMessage());
		CustomEvent.fire('list.section.toggle');
	},
	_setFocusToolbar: function(shouldShowFilter) {
		if (!shouldShowFilter)
			return;
		var toolBarDiv = this._getToolBarDiv();
		if (!toolBarDiv)
			return;
		var firstChild = toolBarDiv.firstChild;
		if (!firstChild)
			return;
		firstChild.focus();
	},
...
e.src = "images/list_v2_filter_reveal.gifx";
	...
	_loadFilter: function(targetDiv, listDiv) {
	       ...
	       ajax.addParam("sysparm_filter_query_prefix", list.filterQueryPrefix || list.widgetID || list.listID || list.listName);
	...
	_getToolBarDiv: function() {
		return this._getByIdAndSuffix("_filterToolbar");
	},
	_getByIdAndSuffix: function(suffix){
		return gel(this.listID + suffix);
	},
	...
	_getFilterToggle: function(list) {
		...
		return list.listContainer.select('button.list_filter_toggle');
	},
	_initEvents: function(list) {
		var toggleFilterButton = this._getFilterToggle(list);
		if (toggleFilterButton.length === 1) {
			if (!this._isFilterEnabled) {
				$j(toggleFilterButton[0])
					.attr('aria-disabled', 'true')
					.attr('disabled', 'disabled')
			} else
				this._initFilterEvents(toggleFilterButton[0]);
		}
	...
	_initFilterEvents: function(elementNode) {
		var self = this;
		$j(elementNode).attr('aria-expanded', self.pinned);
		$j(elementNode).on('click keydown', function(ev) {
			var SPACE_KEY_CODE = 32;
			var validKey = ev.keyCode === SPACE_KEY_CODE || ev.keyCode === Event.KEY_RETURN;
			if (ev.type === 'keydown' && !validKey)
				return;
...
$.trim(that.tagInput.val()).replace( /^s*/, '' ).charAt(0) != '"' ||
					...
					if (!that.tagInput.data('autocomplete-open') && e.key != "Enter") {
						that.createTag(that._cleanedInput());
					}
				});
...
_refresh: function(listTable,  list, loadFlag) {
	...
	_filterDisplay: function(showFlag) {
		...
		var genaiMessage = this._getGenAIFilterMessage();
...
return getMessage('Show / hide natural language filter');
	...
	_getGenAIFilterMessage: function() {
		return gel(this.listID + "nlqGenaiMessagediv");
	},
	checkFilterEnabled: function() {
		var list = this._getList();
		this._isFilterEnabled = list.isFilterEnabled();
