...
window.NOW.FormTags = (function($) {
	...
	function _onTagRemoved(evt, ui, rowId) {
		...
		GlideUXMetrics && GlideUXMetrics.track('Remove tag', options);
	}
	...
	function _triggerUxMetrics(res, action) {
        if (action !== 'add') {
            return;
        }
        var visibility = res.type === 'PRIVATE' ? 'Me' : ((res.type === 'SHARED') ? 'Group' : 'All');
        var options = {
            ID: res.rowId,
            name: res.label,
            recordType: res.table,
            existingTag: 'No',
            visibility: visibility
        };
        GlideUXMetrics && GlideUXMetrics.track('Add Tag', options);
    }
	return {
		init: init
	}
