...
GlideFieldPolicy.prototype = {
	...
	evaluateTerm: function(term) {
		...
		return result;
		function addZeroPadding(dateStr) {
return dateStr.replace(/^\d(?=\s)/, function(d) {
				return '0' + d;
			});
		}
...
var userValue = g_form.getValue(field) + '';
			if (type == 'glide_duration') {
				...
				value = addZeroPadding(value);
				userValue = addZeroPadding(userValue);
			}
...
var userValue = g_form.getValue(term.field) + '';
		...
		if (term.oper == 'ISEMPTY')
			return userValue === '';
		var userDate = getDateFromFormatInPreferenceTimezone(userValue, userFormat);
if (userDate == 0) {
...
if (term.oper == "DATEPART")
		...
		switch (term.oper) {
			...
			case '<':
			case '<=':
				var valueDate = getDateFromFormatInPreferenceTimezone(value, g_user_date_time_format);
			...
			case '>=':
				...
				return (valueDate != 0) && (term.oper === '>=' ? userDate >= valueDate : userDate > valueDate);
			case 'ON':
				var valueDate1 = getDateFromFormatInPreferenceTimezone(values[1], g_user_date_time_format);
				var valueDate2 = getDateFromFormatInPreferenceTimezone(values[2], g_user_date_time_format);
			...
			case 'NOTON':
				...
				return (valueDate1 != 0) && (valueDate2 != 0) && ((userDate < valueDate1) || (userDate > valueDate2));
			case 'BETWEEN':
				var valueDate1 = getDateFromFormatInPreferenceTimezone(values[0], g_user_date_time_format);
				var valueDate2 = getDateFromFormatInPreferenceTimezone(values[1], g_user_date_time_format);
				return (valueDate1 != 0) && (valueDate2 != 0) && (userDate >= valueDate1) && (userDate <= valueDate2);
			case 'LESSTHAN':
	...
	_dateComparisonHelper: function(left, right, comparison, userFormat) {
		...
		var userDate = getDateFromFormatInPreferenceTimezone(userValue, userFormat);
		var theDate = getDateFromFormatInPreferenceTimezone(value, userFormat);
...
var userValue = g_form.getValue(term.field + ".storage") + '';
	...
	_runScript: function(script) {
		if (window.__metadata_trace_enabled)
			window.delegatedActionPushForMetadataTracing('ui_policy', this.source_table, this.sys_id, null);
	...
	_runAction: function(action, result) {
		...
		var valueAction = action.valueAction;
		var value = action.value;
		var fieldMessageType =  action.fieldMessageType;
		var fieldMessage = action.fieldMessage;
		var actionSysId = action.sysId;
		var mappingField = this._getVariableMappingField(field);
		if (mappingField) {
			var uiPolicyActions = {
				mandatory: mandatory,
		...
		if (this.tableName === '(catalog)' && valueAction !== 'ignore' && result) {
			if (valueAction === 'set_value')
				g_form.setValue(field, value);
			else if (valueAction  === 'clear_value')
				g_form.clearValue(field);
			this._debugAction(field, valueAction, result);
		}
		if (this.tableName === '(catalog)' && fieldMessageType !== 'none') {
			if (result) {
				g_form.showFieldMsg(field, fieldMessage, fieldMessageType, false, actionSysId);
				this._debugAction(field, fieldMessageType, result);
			} else {
				g_form.hideFieldMsg(field, false, actionSysId);
				this._debugAction(field, fieldMessageType, result);
			}
		}
		if (mandatory == 'true') {
