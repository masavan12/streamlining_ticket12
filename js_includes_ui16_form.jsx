...
angular.module("ng.amb").service("amb", function (AMBOverlay, $window, $q, $log, $rootScope, $timeout) {
  ...
  var loginWindow = null;
	...
	function addOverrideStyles(el) {
		...
		var backdrop = parent.find('.modal-backdrop').last().get(0);
...
label: "="
		...
		link: function(scope) {
			...
			function setTimeAgo() {
				...
				scope.timeAgoVerbose = [scope.label, timeAgoConverter(scope.timestamp, true, true)].filter(Boolean).join(' ');
			}
...
.factory('snAvatarFactory', function($http, $compile, $templateCache, $q, i18n, snCustomEvent, snConnectService) {
	'use strict';
	return function() {
		var profileMenuText = i18n.getMessage("Open user's profile");
		return {
			restrict : 'E',
			replace: true,
			transclude: true,
...
angular.module('sn.common.controls').directive('snRecordPicker', function($timeout, $http, urlTools, filterExpressionParser, escapeHtml, i18n, select2EventBroker) {
	"use strict";
	var cache = {};
...
' "valueField" : "{{valueField}}", "displayField" : "{{displayField}}"}\' sn-atf-class="builtin:ATF.BaseSNRecordPicker" aria-hidden="true" sn-atf-component-value="{{field}}"/>',
		...
		link: function(scope, element, attrs, ctrl) {
			var TOKENIZER = '####';
			var multipleValueDelimiter = scope.multipleValueDelimiter || ",";
...
OR = "OR";
			...
			if (scope.field.displayValue && scope.multiple)
				scope.field.displayValue = scope.field.displayValue.replace(new RegExp(multipleValueDelimiter, 'g'), TOKENIZER);
...
$select.bind("sn-atf-setvalue", onAtfSetValue);
                    $select.bind("select2-opening", select2Helpers.onOpening);
					scope.$emit('select2.ready', element);
					...
					var select2ResultsId = select2Container.find('ul.select2-results').attr('id');
					select2Container.find(".select2-input").attr("aria-controls", select2ResultsId);
					...
					if (!scope.multiple) {
						...
						closeButton.parent().attr({'draggable': false});
						closeButton.on('mousedown touchend', function(e) {
							e.stopPropagation();
						});
		...
		link: function (scope, element, attrs, ngModel) {
			if (angular.isFunction(element.select2)) {
				...
				$timeout(function(){
					...
					var select2Data = element.data('select2');
					if (select2Data) {
						var select2ResultsId = select2Data.results.attr('id');
						select2Data.focusser.attr("aria-controls", select2ResultsId);
						select2Data.search.attr("aria-controls", select2ResultsId);
					}
...
;
/*! RESOURCE: /scripts/sn/common/controls/service.select2EventBroker.js */
angular.module('sn.common.controls').factory('select2EventBroker', function() {
	var subscribers = {};
	var uuid = -1;
...
angular.module('sn.common.notification').factory('snNotification', function($document, $templateCache, $compile, $rootScope, $timeout, $q, getTemplateUrl, $http, i18n, $sce) {
	'use strict';
...
duration: 5000
	...
	function createNotificationElement(type, message, screenReaderOnly) {
		...
		return getTemplate().then(function(template) {
			...
			thisScope.type = type;
			thisScope.message = type == 'auth' ? $sce.trustAsHtml(message) : message;
			thisScope.icon = icon;
...
angular.module("sn.common.stream").controller("snStream", function($rootScope, $scope, $attrs, $http, nowStream, snRecordPresence, snCustomEvent, userPreferences, $window, $q, $timeout, $sce, snMention, i18n, getTemplateUrl, $injector, dynamicTranslation) {
	...
	function getMembersFromPageContext(members, term) {
		...
		members.forEach(function(member) {
			...
			if (globalMembers[member.sys_id] && globalMembers[member.sys_id].record_is_visible !== member.record_is_visible)
				globalMembers[member.sys_id].record_is_visible = member.record_is_visible;
...
journal.sanitized_new_value = journal.new_value = response.data.result.replace(/\n/g, '<br/>');
			...
			$scope.$on("sn.stream.tap", function() {
				if (POLLING_OFF) {
					stream.fetchStream(onPoll, beforePoll);
					return;
				}
				if (stream)
					stream.tap();
...
var url = "/sys_attachment.do?view=true&sys_id=" + sysId;
			...
			function putUseEmailAmbInWindow() {
				if (!$scope.isFormStream) {
					window.NOW.activity.useEmailAmb = false;
					return;
				}
				if (!$scope.template || $scope.template !== "record_stream_entry.xml") {
					window.NOW.activity.useEmailAmb = false;
					return;
				}
				window.NOW.activity.useEmailAmb = true;
			}
...
[
	...
	"xmlView"
].forEach(key => window.nowapi[key] = window[key]);
...
$j(function() {
	...
	var isOpen = false;
	var keydownEvent = 'keydown.moreOptionsFocusTrap';
	...
	function getTabbableElements($container) {
		return $container.find('a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), [tabindex]:not([tabindex="-1"])').filter(':visible');
	}
	function trapFocus($container) {
		$container.on(keydownEvent, function(e) {
			if (e.keyCode === $j.ui.keyCode.TAB) {
				var $focusable = getTabbableElements($container);
				var $firstFocusable = $focusable?.first();
				var $lastFocusable = $focusable?.last();
				var $currentFocus = $j(':focus');
				if (e.shiftKey) {
					if ($currentFocus?.is($firstFocusable)) {
						e.preventDefault();
						$lastFocusable?.focus();
					}
				}
				else {
					if ($currentFocus?.is($lastFocusable)) {
						e.preventDefault();
