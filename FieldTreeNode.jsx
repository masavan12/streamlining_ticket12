...
item.setNodeClosedImage("images/table.gifx");
	...
	_onClick: function() {
		...
		if (tree.email) {
			...
			if(!tree.tinymce)
			    telemetryForRecordUpdateElement(tree.target);
		}
		else
			value = "current." + this.elementName;
		
		if (this.tree.tinymce && this.tree.tinymce.activeEditor) { 
			this.tree.tinymce.execCommand('mceInsertContent', false, value);
